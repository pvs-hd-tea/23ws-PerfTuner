...
c /= d;
factor = _mm256_set1_pd(c); // load the constant only once outside the loop
int i = lower;
for (; i <= upper - vec_size; i += vec_size) // head loop (optimized using AVX)
{
  vec1 = _mm256_loadu_pd(&A[l*(n+1)+i]);
  vec2 = _mm256_loadu_pd(&A[j*(n+1)+i]);
  vec2 = _mm256_fnmadd_pd(factor,vec1,vec2);
  _mm256_storeu_pd(&A[j*(n+1)+i],vec2);
}
